...
flowchart TD
  ...
  Grammy --> Queue[صف NEKO_OUTBOX_QUEUE]
...
flowchart TD
  ...
  MatchingFlow --> MatchRows[(درخواست‌های match در D1)]
...
sequenceDiagram
  ...
  B->>D1: ذخیره تلاش، پاسخ‌ها و امتیازها
...
```mermaid
flowchart TD
  A[درخواست‌کننده با پروفایل آماده] --> B[Vectorize با topK=30]
  B --> C[فیلتر: نمایش‌پذیری، زبان، ایمنی]
  C --> D[fallback محدود از D1]
  D --> E[رتبه‌بندی قطعی در TypeScript]
  E --> F[نمایش گزینه‌های نزدیک]
  F --> G[نوشتن پیام شروع]
  G --> H[درخواست در انتظار]
  H --> I{پذیرش؟}
...
جزئیات threat model در repository: [نِکونیموس](https://t.me/nekonymous_bot).
...
این نوشته lab مرجع مهندسی پروژه است. برای اطلاعات بیشتر به [نِکونیموس](https://t.me/nekonymous_bot) مراجعه کن.
