...
مشکل از جایی شروع می‌شود که ابزار ناشناس، کم‌کم تبدیل می‌شود به جایی که چیزهای زیادی می‌داند.

یک ربات پیام ناشناس اگر بی‌دقت طراحی شود، می‌تواند خیلی بیشتر از چیزی که لازم دارد ذخیره کند: شناسه‌ها، متن پیام‌ها، ارتباط بین فرستنده و گیرنده، تاریخچه‌ی گفتگو، گزارش‌ها، و داده‌هایی که شاید هیچ‌وقت واقعاً برای عملکرد اصلی سیستم لازم نبوده‌اند.

در چنین سیستمی، مسئله فقط این نیست که «ربات کار می‌کند یا نه». مسئله این است که ربات چقدر باید بداند.

نِکونیموس دور همین سؤال شکل می‌گیرد:

> یک ربات پیام ناشناس، واقعاً چقدر باید از ما بداند؟

هدف فقط ساختن یک ربات ناشناس دیگر نیست. نِکونیموس کمتر ادعا می‌کند، کمتر ذخیره می‌کند، و مرزهایش را واضح‌تر توضیح می‌دهد.
...
وقتی کسی از طریق لینک شخصی تو پیام می‌فرستد، سیستم برای گیرنده یک ticket می‌سازد. این ticket به گیرنده اجازه می‌دهد پیام را ببیند و روی همان ارتباط کارهایی انجام دهد: پاسخ بدهد، فرستنده را مسدود کند، گزارش ثبت کند، یا برای خودش یک نام خصوصی روی آن فرستنده بگذارد.
...
- [Nekonymous: ساخت یک ربات پیام ناشناس و پیشنهاد گفت‌وگو](/fa/lab/anonymous-messaging-on-the-edge)
