...
یک رباتی که اگر قرار است چیزی ذخیره کند، دلیلش روشن باشد. اگر داده‌ای حساس است، محدود و رمزنگاری‌شده بماند. اگر ادعایی درباره حریم خصوصی دارد، از واقعیت فنی خودش بزرگ‌تر حرف نزند.
...
1. تیکتینگ ناشناس با ذخیره‌سازی کمینه و رمزنگاری داده‌های لازم.
2. پیشنهاد گفت‌وگو با پروفایل گفت‌وگو، نه ادعای هماهنگی دقیق.
3. سرویس کوچک و پایدار روی Cloudflare، با هزینه نگهداری پایین و معماری قابل توضیح.

هر سه اصل در نهایت به یک چیز برمی‌گردند: کم کردن فاصله‌ی بین چیزی که محصول ادعا می‌کند و چیزی که واقعاً در معماری اتفاق می‌افتد.

نِکونیموس پیام‌رسانی با رمزنگاری سرتاسری نیست. zero-knowledge نیست. ادعا نمی‌کند ناشناسی بی‌نقص فراهم می‌کند. قرار نیست بگوید هیچ‌کس هیچ‌چیز نمی‌بیند. چون چنین حرفی در یک ربات تلگرام درست نیست.

نِکونیموس یک hosted anonymous relay روی تلگرام است: رباتی که تلاش می‌کند ارتباط بین کاربران را از دید همدیگر پنهان کند، داده‌های ذخیره‌شده را تا حد ممکن کمینه نگه دارد، چیزهایی را که باید ذخیره شوند در حالت ذخیره رمزنگاری کند، و صادقانه بگوید چه چیزی محافظت می‌شود و چه چیزی نه.
...
اما وقتی محصول درباره‌ی پیام ناشناس است، این نگاه خطرناک می‌شود. هر داده‌ای که امروز ذخیره می‌شود، فردا می‌تواند تبدیل شود به ریسک. حتی اگر نیت اولیه بد نباشد، storage خودش یک سطح حمله می‌سازد.
...
flowchart TD
  ...
  B --> C[ساخت پیش‌نویس]
  C --> D[ارسال پیام]
  D --> E[رمزنگاری payload]
  E --> F[TicketVault + inbox pointer]
  F --> G[اعلان به گیرنده]
...
```text
D1
→ کاربران، لینک‌ها، آمار تجمیعی روزانه

TicketVault Durable Object
→ sealed ticket: route_enc + payload_enc

ProfileVault Durable Object
→ پروفایل گفت‌وگوی رمزگذاری‌شده و مسیر بردار

ConversationVault Durable Object
→ suggestion و request sealed، intro رمزگذاری‌شده

PairLedger Durable Object
→ pair lock، cooldown و block — بدون گراف قابل برگشت

UserState Durable Object
→ inbox pointers، پیش‌نویس‌ها، مسدودسازی‌ها، نام‌های خصوصی، جلسه پروفایل

ReportLedger Durable Object
→ گزارش‌های کور (blind tags) — نه transcript در D1

KV
→ فقط کش و مسیر‌یابی سریع، نه منبع اصلی داده

Queue / Outbox / profile-index
→ ارسال‌های غیرحیاتی، idempotent به تلگرام؛ ایندکس غیرهمزمان بردارها
```

این تقسیم‌بندی مهم است، چون همه‌ی داده‌ها یک سطح حساسیت ندارند.

D1 برای داده‌های ساختاری خوب است: کاربر، لینک عمومی، آمار aggregate. اما متن پیام ناشناس، پروفایل، intro یا workflow پیشنهاد گفت‌وگو نباید به شکل plaintext در D1 بنشیند. payload در `TicketVaultDO` نگه داشته می‌شود و بعد از تحویل موفق در صندوق، `payload_enc` پاک می‌شود. برای ادامه‌ی قابلیت‌هایی مثل پاسخ، مسدودسازی، گزارش یا نام خصوصی، `route_enc` تا انقضا باقی می‌ماند — آن هم رمزنگاری‌شده. گیرنده فقط inbox pointer می‌بیند، نه متن plaintext در UserState.
...
payload پیام ناشناس
→ در TicketVaultDO نگه داشته می‌شود، نه به‌عنوان متن ساده در D1.

payload صندوق پیام
→ بعد از تحویل موفق، payload_enc از vault پاک می‌شود.

ticketRef در callback
→ خام در storage ذخیره نمی‌شود؛ sealed pointer و ticketHash نگه داشته می‌شوند.
```
...
نه به معنی رابطه‌یابی.
نه به معنی هماهنگی دقیق.
نه به معنی تشخیص روان‌شناختی.
نه به معنی اینکه سیستم بداند چه کسی برای چه کسی «مناسب» است.

فقط به این معنی که اگر کاربری بخواهد، بتواند یک پروفایل گفت‌وگو بسازد و بر اساس آن، چند گزینه‌ی نزدیک‌تر برای شروع یک مکالمه‌ی ناشناس ببیند.

از نظر محصول، این بیشتر شبیه ترکیب چند سیگنال تقریبی است تا یک حکم قطعی. چند سیگنال از پروفایل گفت‌وگو، چند فیلتر ایمنی، و یک رتبه‌بندی متقابل قابل توضیح کنار هم قرار می‌گیرند تا بگویند «این گزینه‌ها احتمالاً برای شروع گفت‌وگو نزدیک‌ترند»، نه اینکه «این دو نفر با هم هماهنگ‌اند».

برای همین واژه‌ی اصلی محصول «پیشنهاد گفت‌وگو» است. در متن فارسی محصول، اصطلاح اصلی همین است:

> پیشنهاد گفت‌وگو

این تفاوت مهم است. زبان محصول خیلی سریع می‌تواند انتظار رابطه، هماهنگی دقیق یا نتیجه‌ی قطعی بسازد. اما نِکونیموس چنین چیزی نیست. اینجا قرار نیست سیستم بگوید دو نفر با هم جور هستند. قرار است فقط چند گزینه‌ی نزدیک‌تر را برای شروع یک گفت‌وگوی ناشناس پیشنهاد کند.

فرآیند کلی این بخش این‌طور است:

```mermaid
flowchart TD
  A[تکمیل پروفایل گفت‌وگو] --> B[نرمال‌سازی ۸ بُعد]
  B --> C[خلاصه کنترل‌شده]
  C --> D[بردارهای self و desired]
  D --> E[بازیابی دوطرفه با Vectorize]
  E --> F[بارگذاری پروفایل از ProfileVault]
  F --> G[رتبه‌بندی متقابل قطعی]
  G --> H[نوشتن intro]
...
```text
کاربر پروفایل گفت‌وگو را کامل می‌کند (۲۵ سؤال، ۸ بُعد)
→ مقادیر self-style و desired-style نرمال می‌شوند
→ یک خلاصه‌ی کنترل‌شده از پروفایل تولید می‌شود
→ دو بردار ۸بعدی کنترل‌شده ساخته می‌شود — نه embedding متن آزاد
→ Vectorize برای بازیابی coarse دوطرفه استفاده می‌شود
→ ProfileVault پروفایل‌های مرتبط را بارگذاری می‌کند
→ رتبه‌بندی نهایی با منطق متقابل قطعی TypeScript انجام می‌شود
→ اگر کاربر خواست، یک intro کوتاه می‌نویسد
→ طرف مقابل باید request را بپذیرد
→ فقط بعد از پذیرش، گفت‌وگوی ناشناس ساخته می‌شود
```

در این مدل، Vectorize تصمیم نهایی را نمی‌گیرد. فقط برای کشف اولیه استفاده می‌شود: یعنی کمک می‌کند از بین پروفایل‌های فعال، یک مجموعه محدود از گزینه‌ها پیدا شوند. رتبه‌بندی نهایی با منطق مشخص و قابل توضیح داخل کد انجام می‌شود. Workers AI در این مسیر استفاده نمی‌شود.
...
پروفایل گفت‌وگو هم برای برچسب‌زدن به شخصیت طراحی نشده. ۲۵ سؤال در ۸ بُعد — عمق، ریتم پاسخ، مستقیم‌بودن، انرژی، شوخی، همراهی احساسی، سرعت باز شدن، ترمیم سوءتفاهم — سیگنال‌هایی درباره‌ی نحوه‌ی گفت‌وگو و ترجیح فعلی می‌دهد.

نتیجه‌ی این پروفایل حقیقت قطعی درباره‌ی شخصیت کاربر نیست. فقط یک ورودی محصولی است برای اینکه پیشنهادهای گفت‌وگو کمتر تصادفی باشند. تکمیل پروفایل به‌تنهایی discoverability را روشن نمی‌کند؛ کاربر باید نمایش در پیشنهادها را جداگانه فعال کند.
...
درخواست گفت‌وگو
→ فقط وقتی ساخته می‌شود که کاربر یک intro کوتاه بنویسد.

درخواست پذیرفته‌شده
→ وارد همان سیستم تیکتینگ ناشناس می‌شود.

درخواست ردشده یا لغوشده
→ هیچ گفت‌وگویی نمی‌سازد.

Vectorize
→ فقط بازیابی coarse دوطرفه است، نه تصمیم نهایی.

رتبه‌بندی نهایی
→ با منطق متقابل قطعی TypeScript انجام می‌شود.

متن محصول
→ نباید وعده‌ی هماهنگی دقیق، برچسب شخصیت یا رابطه بدهد.
```
...
بعد از refactor تعامل بات، سطح کاربر عمداً کوچک‌تر شده است. منوی اصلی فقط چهار ورودی ثابت دارد:
...
این دکمه‌ها مسیرهای اصلی را باز می‌کنند؛ اما خود تنظیمات، پروفایل گفت‌وگو و پیشنهاد گفت‌وگو با inline keyboard جلو می‌روند. وقتی کاربر در حال نوشتن پیام، پاسخ، نام خصوصی، نام نمایشی یا intro برای یک درخواست گفت‌وگو است، reply keyboard فقط «↩️ لغو» را نشان می‌دهد. این باعث می‌شود متن دکمه‌های منوی اصلی وسط draft با ورودی کاربر اشتباه گرفته نشود.

callbackهای قدیمی هم ترجمه یا شبیه‌سازی نمی‌شوند. اگر کاربر روی دکمه‌ای از نسخه‌ی قبلی در تاریخچه چت بزند، بات فقط می‌گوید آن دکمه دیگر در دسترس نیست و action جدیدی اجرا نمی‌کند.

تصویر کلی ساده است:

```mermaid
flowchart TD
  TG[تلگرام] --> W[Cloudflare Worker]
  W --> D1[(D1: هویت و آمار)]
  W --> TVDO[(TicketVault DO)]
  W --> PVD[(ProfileVault DO)]
  W --> CVD[(ConversationVault DO)]
  W --> PLD[(PairLedger DO)]
  W --> USDO[(UserState DO)]
  W --> RLDO[(ReportLedger DO)]
...
```text
Cloudflare Worker
→ ورودی اصلی ربات، commandها، callbackها، منوی اصلی و منطق محصول

D1
→ کاربران، لینک‌ها، آمار تجمیعی در platform_daily_stats*

TicketVaultDO
→ sealed ticket: route_enc و payload_enc
...
UserStateDO
→ inbox pointers، پیش‌نویس، مسدودسازی، نام خصوصی، جلسه پروفایل

ReportLedgerDO
→ گزارش‌های کور — بدون گراف sender→recipient در D1

KV
→ فقط cache و مسیر‌یابی سریع مثل tg:{hash} و link:{slug}

صف‌ها + TelegramOutboxDO
→ ارسال‌های غیرحیاتی تلگرام، با idempotency و lease

Vectorize
→ بازیابی coarse دوطرفه برای پیشنهاد گفت‌وگو — نه ranker نهایی
```

این جداسازی باعث می‌شود سیستم شبیه یک دیتابیس بزرگ و مبهم نباشد. هر نوع داده جایی قرار می‌گیرد که با نقشش سازگارتر است.

البته این معماری هم جادویی نیست. Cloudflare-native بودن به معنی حذف ریسک یا مقیاس نامحدود نیست. اما برای یک ربات کوچک و جدی، مزیت خوبی دارد: بدون سرور سنتی می‌توان محصول را اجرا کرد، هزینه‌ی نگهداری را پایین نگه داشت، و بخش‌های مختلف storage/queue/vector را به‌صورت managed کنار هم آورد.
...
در محصولاتی که با حریم خصوصی سروکار دارند، ادعا نکردن گاهی مهم‌تر از ادعا کردن است.

نِکونیموس عمداً این‌ها را نمی‌گوید:

| ادعا نمی‌کنیم                     | انجام می‌دهیم                                                       |
| --------------------------------- | ------------------------------------------------------------------- |
| ادعای ناشناسی کامل                | کم‌کردن نشت هویت قابل‌دیدن داخل UI ربات                             |
| حذف کامل مرز اعتماد سرور          | رمزنگاری داده‌های حساس در حالت ذخیره                                |
| پیام‌رسان امن مستقل               | relay ناشناس روی تلگرام با مرز اعتماد روشن                          |
| نگه‌داشتن transcript کامل پیام‌ها | sealed ticket در TicketVaultDO؛ پاک‌کردن `payload_enc` بعد از تحویل |
| تطابق شخصیتی یا هماهنگی دقیق      | پیشنهاد گفت‌وگو بر اساس پروفایل گفت‌وگو                             |
| شروع خودکار مکالمه                | ساخت گفت‌وگو فقط بعد از پذیرش طرف مقابل                             |
| برچسب روان‌شناختی                 | چند سیگنال محصولی برای شروع گفت‌وگوی بهتر                           |
...
D1 پروفایل، intro یا workflow پیشنهاد گفت‌وگو را نگه نمی‌دارد.
Telegram user id خام در D1 نیست.
chat id رمزنگاری شده است.
پروفایل نهایی در ProfileVaultDO رمزگذاری‌شده است.
گزارش‌ها در ReportLedgerDO با tagهای کور نگه داشته می‌شوند، نه در D1.
request و suggestion در ConversationVaultDO sealed هستند؛ intro رمزگذاری‌شده.
...
هدف این نیست که فقط یک ربات deploy شود و روی آن نوشته شود «اعتماد کنید». README، threat model، مستندات conversation suggestions، و متن‌های داخل بات باید یک چیز را بگویند:
...
- منوی اصلی چهارگزینه‌ای و draft mode با دکمه لغو
- پروفایل گفت‌وگو (۲۵ سؤال، ۸ بُعد)
...
نِکونیموس تلاشی برای ساختن یک ربات با ادعای امنیت مطلق نیست.
...
اگر می‌خواهی کد، README، threat model یا مستندات پیشنهاد گفت‌وگو را ببینی:

- [مخزن GitHub](https://github.com/mohetios/Nekonymous)
- [نِکونیموس در تلگرام](https://t.me/nekonymous_bot)

یادداشت فنی مرتبط هم اینجا منتشر شده است:

- [نکونیموس: معماری یک ربات پیام ناشناس با مرزهای روشن](/fa/lab/nekonymous-anonymous-messaging-technical-lab)
