...
<!-- PERSIAN_EDITORIAL_ENGINE_START -->

## Persian editorial work

For any task that creates, rewrites, translates, localizes, audits, or updates Persian-language output:

1. Read `editorial/fa/ENTRYPOINT.md`.
2. Use `editorial/fa/profiles/ali/` as the default author profile unless the task explicitly selects another profile.
3. Load local domain or project context only when it is relevant to the current task.
4. Do not copy project facts, timelines, terminology, or status into the timeless editorial core.
5. Preserve factual meaning, temporal status, ownership, uncertainty, frontmatter, URLs, code, identifiers, placeholders, and runtime-sensitive Markdown.
6. Run the repository editorial checks before completion.

Do not duplicate the Persian editorial rules in this file. The engine under `editorial/fa/` is canonical.

<!-- PERSIAN_EDITORIAL_ENGINE_END -->
