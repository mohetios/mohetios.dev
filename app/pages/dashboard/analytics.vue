...
useMohetSeo({
  title: 'Analytics',
  description: 'Audience, content, search, and performance signals for Mohetios.'
})
...
const tabItems = [
  ...
  { label: 'Search', value: 'search', icon: 'i-lucide-search' },
  { label: 'Referrers', value: 'referrers', icon: 'i-lucide-share-2' },
  { label: 'Performance', value: 'performance', icon: 'i-lucide-gauge' }
]

const rangeItems = ['Last 7 Days', 'Last 30 Days', 'Last 90 Days']
...
const safeTrend = computed(() =>
  analyticsTrend.length ? analyticsTrend : [{ date: '', visitors: 0, pageViews: 0, searchClicks: 0 }]
)
...
const totalPageViews = computed(() =>
  analyticsTrend.reduce((total, point) => total + point.pageViews, 0)
)

const filteredPages = computed(() => {
  const query = search.value.trim().toLowerCase()
  if (!query) return topPages
...
const totalSearchClicks = computed(() =>
  searchQueries.reduce((total, item) => total + item.clicks, 0)
)

const totalSearchImpressions = computed(() =>
  searchQueries.reduce((total, item) => total + item.impressions, 0)
)
...
const avgSearchPosition = computed(() => {
  ...
  const sum = searchQueries.reduce((total, item) => total + Number(item.position), 0)
  return (sum / searchQueries.length).toFixed(1)
})
  ...
  <div class="mx-auto flex w-full max-w-[1600px] flex-col gap-6">
      ...
      <div class="flex min-w-0 items-start gap-3">
        <div>
          ...
          <p class="mt-1 text-sm text-muted">
            Understand audience, content, search, and performance signals.
          </p>
        </div>
      </div>
    ...
    <div v-if="activeTab === 'overview'" class="space-y-4">
      <section class="grid gap-4 xl:grid-cols-5">
        <UCard variant="outline" :ui="dashboardCardUi" class="flex flex-col xl:col-span-3">
          <template #header>
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p class="mt-1 text-sm text-muted">
                  Visitors across the selected date range.
                </p>
              </div>
          ...
          <div class="h-64 min-h-48">
            <svg
              ...
              class="h-full w-full"
              role="img"
              aria-label="Audience trend over the selected date range"
            >
              <line
        ...
        <UCard variant="outline" :ui="dashboardCardUi" class="xl:col-span-2">
          ...
          <ul class="divide-y divide-default">
            <li
              v-for="page in topPages.slice(0, 5)"
              :key="page.path"
      ...
      <section class="grid gap-4 lg:grid-cols-3">
        <UCard variant="outline" :ui="dashboardCardUi">
          <template #header>
            <h2 class="text-base font-semibold text-highlighted">Referrers</h2>
          </template>

          <ul class="space-y-4">
            <li v-for="item in referrers" :key="item.source">
        ...
        <UCard variant="outline" :ui="dashboardCardUi">
          <template #header>
            <h2 class="text-base font-semibold text-highlighted">Countries</h2>
          </template>
        ...
        <UCard variant="outline" :ui="dashboardCardUi">
          <template #header>
            <h2 class="text-base font-semibold text-highlighted">Web Vitals</h2>
          </template>
    ...
    <div v-else-if="activeTab === 'content'" class="space-y-4">
      <section class="flex flex-col gap-3 rounded-2xl border border-default bg-default p-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 class="text-sm font-semibold text-highlighted">Top content</h2>
          <p class="text-xs text-muted">{{ filteredPages.length }} of {{ topPages.length }} pages</p>
        </div>
        <UInput
          v-model="search"
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-default text-left text-xs uppercase tracking-wide text-muted">
                ...
                <th class="px-4 py-3 font-medium">Top Source</th>
              </tr>
            </thead>
      ...
      <UCard variant="outline" :ui="{ ...dashboardCardUi, body: 'p-0' }">
        <template #header>
          <div class="flex items-center gap-2">
            ...
            <h2 class="text-base font-semibold text-highlighted">Search queries</h2>
          </div>
        </template>
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-default">
              <tr v-for="item in searchQueries" :key="item.query" class="transition hover:bg-muted/30">
    ...
    <div v-else-if="activeTab === 'referrers'" class="space-y-4">
      <section class="grid gap-4 lg:grid-cols-2">
        <UCard variant="outline" :ui="dashboardCardUi">
          <template #header>
            <h2 class="text-base font-semibold text-highlighted">Top Referrers</h2>
          </template>
            ...
            <li v-for="item in referrers" :key="item.source">
              <div class="flex items-center justify-between text-sm">
                ...
                <span class="flex items-center gap-2 text-muted">
                  ...
                  <UBadge color="success" variant="soft" size="xs" class="rounded-full">
                    {{ item.trend }}
                  </UBadge>
                </span>
              </div>
        ...
        <UCard variant="outline" :ui="dashboardCardUi">
          <template #header>
            <h2 class="text-base font-semibold text-highlighted">Top Countries</h2>
          </template>
    ...
    <div v-else-if="activeTab === 'performance'" class="space-y-4">
      ...
      <section class="grid gap-4 lg:grid-cols-3">
        <UCard variant="outline" :ui="dashboardCardUi" class="lg:col-span-1">
          <template #header>
            <h2 class="text-base font-semibold text-highlighted">Page Load Time</h2>
          </template>

          <div class="flex items-end gap-2">
            <p class="text-4xl font-semibold tracking-tight text-highlighted">1.4s</p>
            <span class="pb-1 text-sm font-medium text-primary">↓ 8.2%</span>
          </div>
          <p class="mt-2 text-xs text-muted">Average across all pages, faster than the prior period.</p>
          <UProgress :model-value="72" :max="100" size="sm" class="mt-4" />
        </UCard>

        <UCard variant="outline" :ui="dashboardCardUi" class="lg:col-span-2">
          <template #header>
            <h2 class="text-base font-semibold text-highlighted">Cache / Edge summary</h2>
          </template>
          ...
          <div class="grid gap-3 sm:grid-cols-3">
            <div class="rounded-xl border border-default bg-muted/20 p-3">
              ...
              <p class="mt-1 text-lg font-semibold text-highlighted">—</p>
            </div>
            <div class="rounded-xl border border-default bg-muted/20 p-3">
              <div class="flex items-center gap-1.5 text-xs text-muted">
          ...
          <p class="mt-3 text-xs text-muted">
            Edge metrics will populate once Cloudflare Analytics is connected.
          </p>
        </UCard>
      </section>
      ...
      <UCard variant="outline" :ui="dashboardCardUi" class="border-dashed">
        <div class="flex items-start gap-3">
          ...
          <div>
            <h3 class="text-sm font-semibold text-highlighted">Cloudflare integration</h3>
            <p class="mt-1 text-sm text-muted">
              Connect Cloudflare Analytics later to show real requests, cache hit ratio, edge errors,
              and Worker activity.
            </p>
          </div>
        </div>
    ...
    <section class="rounded-2xl border border-dashed border-default bg-muted/20 p-4">
      <div class="flex items-start gap-3">
        ...
        <div>
          <p class="text-sm font-medium text-highlighted">Data source</p>
          <p class="mt-1 text-sm text-muted">
            This page currently uses mock analytics data. Later it can connect to Cloudflare Web
            Analytics, Cloudflare Analytics, Workers metrics, and Google Search Console.
          </p>
        </div>
      </div>
    </section>
